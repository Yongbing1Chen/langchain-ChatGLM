...
from chains.milvusdb_qa import MilvusDBQA, exists_vector_store
...
async def upload_file(
    ...
    knowledge_base_id, loaded_files = milvusdb_qa.init_knowledge_vector_store([file_path], knowledge_base_id)
...
async def upload_files(
    ...
    if filelist:
        knowledge_base_id, loaded_files = milvusdb_qa.init_knowledge_vector_store(filelist, knowledge_base_id)
        if len(loaded_files):
...
async def delete_doc(
    ...
    if os.path.exists(doc_path):
        ...
        if len(remain_docs.data) == 0:
            ...
            status = milvusdb_qa.delete_collection_from_vector_store(doc_path, knowledge_base_id)
...
async def update_doc(
    ...
    if not os.path.exists(doc_path):
        ...
        knowledge_base_id, loaded_files = milvusdb_qa.init_knowledge_vector_store([file_path], knowledge_base_id,drop_old=True)
        if len(loaded_files) > 0:
...
async def local_doc_chat(
    ...
    if exists_vector_store(knowledge_base_id):
        ...
        for resp, history in milvusdb_qa.get_knowledge_based_answer(
                query=question, knowledge_base_id=knowledge_base_id, chat_history=history, streaming=True
        ):
            pass
        source_documents = [
...
async def bing_search_chat(
    ...
    for resp, history in milvusdb_qa.get_search_result_based_answer(
...
async def chat(
    ...
    for answer_result in milvusdb_qa.llm.generatorAnswer(prompt=question, history=history,
                                                          streaming=True):
...
def api_start(host, port):
    ...
    app.websocket("/milvusdb_qa/stream-chat/{knowledge_base_id}")(stream_chat)

    app.get("/", response_model=BaseResponse)(document)

    app.post("/chat", response_model=ChatMessage)(chat)

    app.post("/milvusdb_qa/upload_file", response_model=BaseResponse)(upload_file)
    app.post("/milvusdb_qa/upload_files", response_model=BaseResponse)(upload_files)
    app.post("/milvusdb_qa/local_doc_chat", response_model=ChatMessage)(local_doc_chat)
    app.post("/milvusdb_qa/bing_search_chat", response_model=ChatMessage)(bing_search_chat)
    app.get("/milvusdb_qa/list_knowledge_base", response_model=ListDocsResponse)(list_kbs)
    app.get("/milvusdb_qa/list_files", response_model=ListDocsResponse)(list_docs)
    app.delete("/milvusdb_qa/delete_knowledge_base", response_model=BaseResponse)(delete_kb)
    app.delete("/milvusdb_qa/delete_file", response_model=BaseResponse)(delete_doc)
    app.post("/milvusdb_qa/update_file", response_model=BaseResponse)(update_doc)

    milvusdb_qa = MilvusDBQA()
    milvusdb_qa.init_cfg(
